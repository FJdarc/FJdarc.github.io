...
[原文](https://wikis.khronos.org/opengl/Shader_Compilation)
...
**注意**：本文包含对OpenGL 4.x功能的引用，例如[曲面细分着色器](https://wikis.khronos.org/opengl/Tessellation)和[计算着色器](https://wikis.khronos.org/opengl/Compute_Shader)。如果您使用的是OpenGL 3.x，则可以忽略这些引用。
...
[程序对象](https://wikis.khronos.org/opengl/GLSL_Object#Program_objects)可以包含所有[着色器](https://wikis.khronos.org/opengl/Shader)阶段的可执行代码，因此渲染时只需绑定一个程序对象即可。构建包含多个着色器阶段的程序需要两阶段编译过程。

这种两阶段编译过程类似于C和C++源代码的标准编译/链接流程。C/C++源代码首先通过编译器编译，生成目标文件。要获得可执行代码，必须将一个或多个目标文件链接在一起。

采用这种程序创建方法，着色器代码首先通过编译器编译，生成 _着色器对象_。要获得可执行程序对象，必须将一个或多个着色器对象链接在一起。

### 着色器对象编译

第一步是为每个要使用的着色器创建着色器对象并编译它们。要创建着色器对象，请调用以下函数：

GLuint [glCreateShader](https://wikis.khronos.org/opengl/GLAPI/glCreateShader)(GLenum shaderType​);

此函数会为给定 _shaderType_​ 指定的着色器阶段创建一个空的着色器对象。着色器类型必须是`GL_VERTEX_SHADER`、`GL_TESS_CONTROL_SHADER`、`GL_TESS_EVALUATION_SHADER`、`GL_GEOMETRY_SHADER`、`GL_FRAGMENT_SHADER`或`GL_COMPUTE_SHADER`之一。请注意，控制着色器和评估着色器需要 GL 4.0（或[ARB_tessellation_shader](https://registry.khronos.org/OpenGL/extensions/ARB/ARB_tessellation_shader.txt)），而计算着色器需要 GL 4.3（或[ARB_compute_shader](https://registry.khronos.org/OpenGL/extensions/ARB/ARB_compute_shader.txt)）。

创建着色器对象后，您需要为其提供表示[GLSL](https://wikis.khronos.org/opengl/OpenGL_Shading_Language)源代码的实际文本字符串。这可以通过以下函数完成：
...
此函数接受由 _string_​ 指定的字符串数组，并将其存储到 _shader_​ 中。任何先前存储的字符串都将被移除。_count_​ 是单个字符串的数量。OpenGL会将这些字符串复制到内部内存中。

着色器编译时，会将所有给定的字符串连接起来进行编译。这使得用户可以轻松地从文件中加载大部分着色器代码，同时还可以为一组着色器添加标准化的前缀。

_length_​ 可以是NULL或包含 _count​_ 个整数的数组。这些整数是 _string​_ 数组中相应字符串的长度。这允许您使用非NULL终止的字符串。如果您传递NULL，则OpenGL将假定所有字符串都是以NULL结尾的，并以通常的方式计算长度。

将着色器字符串设置到着色器对象后，可以使用以下函数对其进行编译：
...
编译可能成功也可能失败。着色器编译失败不是[OpenGL错误](https://wikis.khronos.org/opengl/OpenGL_Error)；您需要专门检查它。这可以通过调用[glGetShaderiv](https://wikis.khronos.org/opengl/GLAPI/glGetShader)函数来实现：

GLint success = 0;
glGetShaderiv(shader, GL_COMPILE_STATUS, &success);

如果 _success_ 为`GL_FALSE`，则表示最近一次编译失败。否则，表示编译成功。

着色器编译是成功或失败的，但了解失败的原因通常很有用。与大多数编程语言一样，这些信息以文本消息的形式提供。OpenGL允许您查询包含此信息的日志。首先，您必须使用[glGetShaderiv](https://wikis.khronos.org/opengl/GLAPI/glGetShader)查询日志的长度：

GLint logSize = 0;
glGetShaderiv(shader, GL_INFO_LOG_LENGTH, &logSize);

这告诉您需要分配多少字节；长度包含NULL终止符。获取长度并分配足够的内存后，您可以使用此函数获取日志：
...
_maxLength_ 是 _infoLog_ 的大小；它告诉OpenGL最多可以写入 _infoLog_ 的字节数。_length_ 是返回值，指定实际写入 _infoLog_ 的字节数；如果您不需要此信息，可以传递NULL。
...
// maxLength 包含 NULL 字符
std::vector<GLchar> errorLog(maxLength);
glGetShaderInfoLog(shader, maxLength, &maxLength, &errorLog[0]);

// 以您认为最佳的方式提供信息日志。 
// 退出并返回失败。 
glDeleteShader(shader); // 防止着色器内存泄漏。 
return;
}
...
成功编译所需的着色器对象后，即可将它们链接到一个程序中。首先，使用以下命令创建一个程序对象：

GLuint [glCreateProgram](https://wikis.khronos.org/opengl/GLAPI/glCreateProgram)();

该函数不接受任何参数。

创建程序后，必须将要链接到该程序的着色器对象附加到程序中。这可以通过以下函数完成：

void [glAttachShader](https://wikis.khronos.org/opengl/GLAPI/glAttachShader)(GLuint program​, GLuint shader​);

可以多次调用此函数，每次使用不同的 _着色器对象_。

**注意**：可以将多个用于[同一着色器阶段](https://wikis.khronos.org/opengl/Shader)的着色器对象附加到程序中。这是完全合法的。链接时，所有代码将被组合在一起。但是，这些着色器对象中只能有一个包含main函数。这种链接方式与在C/C++程序中链接多个目标文件非常相似：一个.cpp文件如果包含函数的向前声明，就可以调用另一个.cpp文件中的代码。因此，您可以拥有函数“库”，各个着色器可以选择使用这些库中的函数。

**建议**：尽管可以使用此功能，但最好不要使用它。它通常可以正常工作，但由于大多数OpenGL应用程序不使用此功能，因此它不像OpenGL API的其他部分那样经过彻底测试。因此，您更有可能遇到驱动程序错误。通常情况下，每个着色器阶段只使用一个着色器对象即可。

### 链接前准备

可以设置一些参数来影响链接过程。这通常涉及与程序接口相关的设置，包括：

- [顶点着色器输入属性位置](https://wikis.khronos.org/opengl/Vertex_Shader#Inputs)。 
- [片段着色器输出颜色编号](https://wikis.khronos.org/opengl/Fragment_Shader#Output_buffers)。 
- [变换反馈输出捕获](https://wikis.khronos.org/opengl/Transform_Feedback#Shader_setup)。 
- [程序分离](https://wikis.khronos.org/opengl/Shader_Compilation#Separate_programs)。

链接后无法更改这些值；如果在链接前未设置这些值，则之后也无法设置。

### 程序链接

链接失败的原因有很多，包括但不限于：

- [程序中两个着色器阶段之间的匹配无效](https://wikis.khronos.org/opengl/Shader_Compilation#Interface_matching)。 
- 违反各种着色器阶段的限制。其中一些可以在编译时检测到，但另一些则必须等到链接时才能检测到。 
- 在不同的着色器阶段中，两个或多个[某些类型的全局定义](https://wikis.khronos.org/opengl/Shader_Compilation#Linking_and_variables)具有相同的名称，但定义却不同。 
- 引用了已声明但未定义的函数。

程序链接失败的[检测和处理方式](https://wikis.khronos.org/opengl/Shader_Compilation#Error_handling)与着色器编译失败类似。

程序成功链接后即可使用。

### 链接与变量

通常情况下，不同着色器阶段的着色器对象彼此独立。每个着色器阶段的代码都是独立的，拥有各自的全局变量、函数等等。

但情况并非完全如此。某些定义在着色器阶段之间是共享的。具体来说，这些共享定义包括[uniform变量](https://wikis.khronos.org/opengl/Uniform_(GLSL))、[缓冲区变量](https://wikis.khronos.org/opengl/Shader_Storage_Buffer_Object)和缓冲区支持的接口块。

如果在某个阶段定义了其中一个对象，则另一个阶段可以使用相同的名称和完全相同的定义来定义同一个对象。如果发生这种情况，那么通过[内省API](https://wikis.khronos.org/opengl/Program_Introspection)只能看到一个uniform变量/缓冲区变量/接口块。因此，同一程序中的着色器阶段可以共享uniform变量，从而允许使用一次[glUniform](https://wikis.khronos.org/opengl/GLAPI/glUniform)调用为两个阶段设置相同的值。

但是，要实现这一点，这些定义必须完全相同。这包括成员的顺序、它们使用的任何用户定义数据结构、数组大小等等。如果不同阶段的两个定义具有相同的名称但定义不同，则会发生链接错误。

### 清理

链接（无论成功与否）之后，最好将所有着色器对象从程序中分离。这可以通过以下函数完成：

void [glDetachShader](https://wikis.khronos.org/opengl/GLAPI/glDetachShader)(GLuint program, GLuint shader);

_shader_ 必须之前已附加到 _program_。

如果您不打算在链接其他程序时使用此特定着色器对象，则可以将其删除。这可以通过[glDeleteShader](https://wikis.khronos.org/opengl/GLAPI/glDeleteShader) 函数完成。请注意，着色器的删除操作会延迟到该着色器对象不再附加到任何程序时才会执行。因此，在链接后分离着色器是一个好习惯。
...
```c
// 将着色器代码读取到相应的缓冲区
std::string vertexSource = // 获取顶点着色器的源代码。
std::string fragmentSource = // 获取片段着色器的源代码。

// 创建一个空的顶点着色器句柄
GLuint vertexShader = glCreateShader(GL_VERTEX_SHADER);

// 将顶点着色器源代码发送到 GL
// 注意：std::string 的 .c_str() 方法返回的字符串以 NULL 字符结尾。
...
// maxLength 包含 NULL 字符
std::vector<GLchar> infoLog(maxLength); glGetShaderInfoLog(fragmentShader, maxLength, &maxLength, &infoLog[0]);

// 我们不再需要着色器了。
glDeleteShader(fragmentShader);
// 两个着色器都不需要了。避免着色器内存泄漏。
glDeleteShader(vertexShader);
...
// 注意这里使用了不同的函数：glGetProgram* 而不是 glGetShader*。
GLint isLinked = 0;
